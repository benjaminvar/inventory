...
<ValidationObserver tag="form" @submit.prevent v-slot="{ invalid }">
    ...
    <div class="form-group">
      <ValidationProvider rules="required" name="Stock" v-slot="{ errors }">
    ...
    <div class="form-group">
      <ValidationProvider rules="required" name="Min Stock" v-slot="{ errors }">
        <label for>Min. Stock</label>
        <input type="text" class="form-control" v-model="product.min_stock" />
        <div class="text-danger mt-2"><span>{{ errors[0] }}</span></div>
      </ValidationProvider>
    </div>
    <div class="form-group">
      <ValidationProvider rules="required" name="Provider" v-slot="{ errors }">
        <label for>Provider</label>
        <select class="form-control" v-model="product.provider_id">
     ...
     <div class="form-group">
      <ValidationProvider rules="required" name="Image" v-slot="{ errors }">
        ...
        <input type="file" class="form-control" @change="handleFile($event)" />
        <div class="text-danger mt-2"><span>{{ errors[0] }}</span></div>
      </ValidationProvider>
    </div>
...
export default {
  ...
  methods: {
    async loadData() {
      let responseProduct = await this.$http.get(`ajax/products/${this.itemId}`);
